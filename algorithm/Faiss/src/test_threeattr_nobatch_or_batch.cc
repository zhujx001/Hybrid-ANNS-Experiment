...
std::pair<std::map<std::string, DatasetConfig>, std::map<std::string, QuerySetConfig>> get_query_config() {
    std::map<std::string, DatasetConfig> datasets = {
        ...
        {"audio", {
            ...
            "IVF70,Flat",
            {
            1, 2, 4, 5, 6, 7, 8, 10, 12, 15, 18, 20, 25, 30, 35, 40, 45, 50
            },
            {16, 1}
        }},
        {"enron", {
   ...
   std::map<std::string, QuerySetConfig> query_sets = {
    ...
    // // {"2-1", {"多属性构建单标签搜索", {0}, "query_set_2_1"}},
    // // {"2-2", {"多属性构建单标签搜索", {0}, "query_set_2_2"}},
    // {"3-1", {"1%选择性实验", {15}, "query_set_3_1"}},
    // {"3-2", {"25%选择性实验", {16}, "query_set_3_2"}},
    // {"3-3", {"50%选择性实验", {17}, "query_set_3_3"}},
    // {"3-4", {"75%选择性实验", {18}, "query_set_3_4"}},
    // {"4", {"稀疏属性实验", {7}, "query_set_4"}},
    // {"5-1", {"长尾分布标签实验", {1}, "query_set_5_1"}},
    // {"5-2", {"正态分布标签实验", {2}, "query_set_5_2"}},
    // {"5-3", {"幂律分布标签实验", {4}, "query_set_5_3"}},
    // {"5-4", {"均匀分布标签实验", {0}, "query_set_5_4"}}
...
void runAllExperiments(int thread_nums, int cycle_num, bool is_save_to_file, bool isBatch) {
    ...
    if(thread_nums == 5){
        dataset_order = {"audio", "enron",  "msong"};
    }
    else{
        dataset_order = {"audio", "enron", "glove-100", "sift", "msong", "gist"};
    }
    ...
    for (const auto& dataset_name : dataset_order) {
        ...
        experiment.setBaseFilePaths(
            dataDir + "labels/" + dataset_name + "/labels_with_selectivity.txt",
        ...
        for (const auto& [query_set_name, query_set_config] : query_sets) {
            ...
            if(query_set_name == "6"){
                if(dataset_name == "audio"){
                    experiment.setNprobeValues({1, 2, 4, 5, 6, 7, 8, 10, 12, 15, 18, 20, 25, 30, 35, 40, 45, 50, 60});
                }
                else if(dataset_name == "gist"){
                    experiment.setNprobeValues({
                        6, 8, 10,
                        15, 20, 25, 30, 35, 40, 45, 50,     
                        60, 70, 80, 90, 100,            
                ...
                else if(dataset_name == "glove-100"){
                    experiment.setNprobeValues({
                        6, 8, 10,
                        15, 20, 25, 30, 35, 40, 45, 50,     
                        60, 70, 80, 90, 100,            
            ...
            else if(query_set_name == "7_1"){
                if(dataset_name == "audio"){
                    experiment.setNprobeValues({6, 8, 15, 20, 25,  35, 41, 45, 50, 55, 60, 63, 66, 69, 70});
                }
                else if(dataset_name == "enron"){
                    experiment.setNprobeValues({4, 7, 12, 15, 20, 27, 30, 35, 40, 45, 50, 55, 60, 63, 65});
                }
                else if(dataset_name == "gist"){
                    experiment.setNprobeValues({
                        6, 8, 10,
                        15, 20, 25, 30, 35, 40, 45, 50,     
                        60, 70, 80, 90, 100,            
                        120, 140, 160, 180, 200, 230, 260, 300, 350, 400, 450, 500, 550, 600
                        });
                }
                else if(dataset_name == "glove-100"){
                    experiment.setNprobeValues({
                    ...
                    experiment.setNprobeValues({
                        5, 6, 8, 10, 12,
                        15, 20, 25, 30, 35, 40, 45, 50,
                        70, 90, 100, 120, 150, 200, 250, 300, 350
                        });
                }
                    ...
                    experiment.setNprobeValues({
                        15, 35,            
                        60, 90,         
                        120, 140, 160, 180, 200, 230, 260, 300, 350, 400, 450
                        });
                }
            }
