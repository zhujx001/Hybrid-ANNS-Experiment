...
public:
    ...
    Query(const uint16_t &attr1, const uint16_t &attr2, const uint16_t &attr3, const std::vector<float> &queryVec)
        : attribute1(attr1),
          attribute2(attr2),
          attribute3(attr3),          
    ...
    bool match(const DataPointLabel &dataPoint) const
    {
        if (attribute1 == dataPoint.attribute1 &&
            attribute2 == dataPoint.attribute2 &&
            attribute3 == dataPoint.attribute3)
        {
            return true;
        }
        return false;
    ...
    {
        return std::to_string(attribute1) + "_" + std::to_string(attribute2) + "_" + std::to_string(attribute3);
    }
